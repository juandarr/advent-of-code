...
def shoelace(groundDug):
    ...
    total = 0
    while (i>=0):
        total += groundDug[i][0]*groundDug[i-1][1]-groundDug[i][1]*groundDug[i-1][0]
        i-=1
        if (i==0):
            total += groundDug[i][0]*groundDug[len(groundDug)-2][1]-groundDug[i][1]*groundDug[len(groundDug)-2][0]
            break
    return total//2
    ...
    for instruction in m:
        ...
        for idx in range(len(hexValue)-1,-1,-1):
            if hexValue[idx] in hexToDec:
                value += hexToDec[hexValue[idx]]*(16**(l-idx))
            else:
                value += int(hexValue[idx])*(16**(l-idx))
        # End: part 2 specific
        i += d[0]*value
        j += d[1]*value
        edge+=value

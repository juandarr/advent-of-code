                ...
                if prevCost>newCost:
                    ...
                    if newNode ==target:
                        if mini>newCost:
                            mini = newCost
        if mini == float('inf'):
            return ','.join([str(i) for i in blk])
    return ''

def main(filename):
    b,dim = parseInformation(filename)
    coord = shortestPath(b,dim)
    return coord
...
if __name__ == "__main__":
    ...
    if test:
        performTests(2024, 18, ['6,1'],main) 
    else:
        coord = getAnswer(2024, 18, main)
        print("The coordinate of the first byte blocking the exit is: {0}".format(coord))